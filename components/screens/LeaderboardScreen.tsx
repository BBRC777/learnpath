...
import { getLeaderboard, getLevelInfo } from '@/lib/db'
      ...
      <div style={{ maxWidth:640, margin:'0 auto', padding:'24px 28px 60px' }}>
            ...
            {[1, 0, 2].map(rank => {
              ...
              return (
                <div key={rank} style={{ display:'flex', flexDirection:'column' as const, alignItems:'center', flex:1 }}>
                  ...
                  <div style={{ width:44, height:44, borderRadius:'50%', background: isMe ? 'var(--amber)' : 'var(--bg3)', border:`2px solid ${medalColors[rank]}`, display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--serif)', fontSize:18, color: isMe ? '#0a0b0f' : 'var(--amber)', marginBottom:6 }}>
                  ...
                  <div style={{ width:'100%', height:heights[rank], background:`${medalColors[rank]}22`, border:`1px solid ${medalColors[rank]}44`, borderRadius:'6px 6px 0 0', display:'flex', alignItems:'center', justifyContent:'center' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column' as const, gap:6 }}>
          {leaders.map((leader, i) => {
            const isMe = leader.id === currentUserId
            const info = getLevelInfo(leader.xp || 0)
            const badges: string[] = leader.badges || []
            return (
              <div key={leader.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px', borderRadius:10, background: isMe ? 'var(--amber-bg)' : 'var(--bg2)', border:`1px solid ${isMe ? 'rgba(212,133,58,0.4)' : 'var(--border)'}` }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:13, fontWeight:500, color: isMe ? 'var(--amber)' : 'var(--text)', marginBottom:1 }}>
                    {isMe ? 'You' : (leader.display_name || 'Learner')}
                    {isMe && <span style={{ marginLeft:6, fontSize:9, fontFamily:'var(--mono)', color:'var(--amber2)' }}>← you</span>}
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                    <span style={{ fontSize:10, fontFamily:'var(--mono)', color:'var(--text3)' }}>L{info.level} {info.title}</span>
                    {leader.streak > 0 && <span style={{ fontSize:10, color:'var(--amber)', fontFamily:'var(--mono)' }}>🔥 {leader.streak}</span>}
                    {badges.slice(0,3).map((b: string, bi: number) => (
                      <span key={bi} style={{ fontSize:12 }}>{['🎯','🔥','⚡','💎','⭐','📚','🎓','👑','🗺️'][Math.abs(b.charCodeAt(0)) % 9]}</span>
                    ))}
                  </div>
                </div>
