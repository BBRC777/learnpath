...
export default function CurriculumScreen() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (userId) {
        setSaving(true)
        try { const saved = await saveCurriculum(userId,{topic,level,durLabel:duration.label,days:activeDays.length,time:sessionTime,style:styles.join(', '),curriculum:parsed}); setSavedId(saved.id); setPathCount(c=>c+1) }
  ...
  if (showPaywall) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100%', padding:24 }}>
      <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:18, padding:'36px 40px', width:'100%', maxWidth:460, textAlign:'center' as const }}>
        ...
        <div style={{ fontSize:13.5, color:'var(--text2)', lineHeight:1.65, marginBottom:24 }}>You have used your {FREE_MAX_PATHS} free learning paths. Upgrade to Pro for unlimited curricula, paths up to 12 weeks, Study Mode, and the AI Tutor.</div>
        <div style={{ background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:10, padding:'14px 18px', marginBottom:24, textAlign:'left' as const }}>
          ...
          {['Unlimited learning paths','Paths up to 12 weeks','Study Mode','AI Tutor','Priority support'].map((f,i) => (
    ...
    <div style={{ overflowY:'auto', height:'100%' }}>
      <div style={{ maxWidth:740, margin:'0 auto', padding:'28px 32px 80px' }}>
        ...
        <div style={{ display:'flex', gap:10, marginBottom:24 }}>
          ...
          <button onClick={() => { setCurriculum(null); setSavedId(null); setStep(0) }} style={{ flex:1, padding:13, borderRadius:10, border:'1px solid var(--border)', background:'var(--bg2)', color:'var(--text2)', fontFamily:'var(--sans)', fontSize:13, cursor:'pointer' }}>Build Another</button>
        </div>
