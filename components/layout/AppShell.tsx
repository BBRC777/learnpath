  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:'var(--bg)', color:'var(--text)' }}>
      ...
      <main style={{ marginLeft:sidebarW, flex:1, display:'flex', flexDirection:'column', minHeight:'100vh', transition:'margin-left 0.2s ease' }}>
        ...
        {/* Topbar */}
        <div style={{ borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', padding:'10px 12px', background:'var(--bg2)', flexShrink:0, position:'sticky', top:0, zIndex:40, flexWrap:'wrap', rowGap:8, columnGap:10 }}>
            ...
            {!isPro && (
              <div style={{ background:'var(--amber-bg)', border:'1px solid rgba(212,133,58,0.3)', borderRadius:8, padding:'12px 14px', marginBottom:14 }}>
                ...
                <div style={{ display:'flex', gap:8 }}>
                  <button onClick={()=>window.open(getUpgradeUrl(user?.id), '_blank')} style={{ padding:'8px 10px', borderRadius:7, border:'1px solid rgba(212,133,58,0.6)', background:'rgba(212,133,58,0.15)', color:'var(--amber)', fontFamily:'var(--sans)', fontSize:11, cursor:'pointer', flex:1, textAlign:'center' as const, position:'relative' as const }}>
                  ...
                  <button onClick={()=>window.open(getUpgradeUrl(user?.id), '_blank')} style={{ padding:'8px 10px', borderRadius:7, border:'1px solid var(--border2)', background:'var(--bg3)', color:'var(--text2)', fontFamily:'var(--sans)', fontSize:11, cursor:'pointer', flex:1, textAlign:'center' as const }}>
