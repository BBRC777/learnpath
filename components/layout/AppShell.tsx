...
export default function AppShell({ children }: AppShellProps) {
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:'var(--bg)', color:'var(--text)' }}>
      ...
      {/* SIDEBAR */}
      <aside style={{ position:'fixed', left:0, top:0, width:sidebarW, height:'100vh', background:'var(--bg2)', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', overflowY:'auto', zIndex:50 }}>

        {/* Logo */}
        <div style={{ padding:'20px 18px 15px', borderBottom:'1px solid var(--border)' }}>
          <div style={{ fontFamily:'var(--serif)', fontSize:20, color:'var(--amber)', letterSpacing:'-0.01em' }}>◆ Learnpath</div>
          <div style={{ fontFamily:'var(--mono)', fontSize:9, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'0.12em', marginTop:2 }}>Learn Anything · All Inside</div>
        </div>
          ...
          {NAV.map(item => {
            ...
            return (
              <div key={item.href} onClick={() => !isLocked && router.push(item.href)}
                style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 10px', borderRadius:8, marginBottom:2, cursor:isLocked?'default':'pointer', background:active?'var(--amber-bg)':'transparent', color:active?'var(--amber)':isLocked?'var(--text3)':'var(--text2)', fontSize:13, transition:'all 0.12s' }}
        ...
        {/* Streak */}
        <div style={{ margin:'8px 8px 0', padding:'10px 12px', background:'var(--amber-bg)', border:'1px solid rgba(212,133,58,0.25)', borderRadius:10 }}>
          ...
          <div style={{ fontSize:10, fontFamily:'var(--mono)', color:'var(--text3)', marginTop:1 }}>Keep the chain alive!</div>
        </div>
        ...
        {/* User footer */}
        <div style={{ padding:'10px 8px 12px', borderTop:'1px solid var(--border)', marginTop:8 }}>
          <div onClick={() => { setShowSettings(true); setEditName(profile?.display_name||'') }}
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:10, fontFamily:'var(--mono)', color:'var(--text3)', marginTop:1 }}>{profile?.is_pro?'Pro':'Free'} · {profile?.streak??0}/1 paths</div>
            </div>
            <span style={{ fontSize:11, color:'var(--text3)' }}>Settings</span>
          </div>
        </div>
      </aside>
